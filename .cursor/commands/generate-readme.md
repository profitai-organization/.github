...
3. **Nome do arquivo da logo:** listar o conteúdo de `assets/logo/` e identificar o arquivo que corresponda à escolha (ex.: se escolheu "text", usar o arquivo cujo nome contém "text", como `logo-text.jpg` ou `logo-text.png`). No README em `profile/README.md`, usar no `src` da imagem o caminho **`../assets/logo/<nome-do-arquivo>`** com o **nome completo do arquivo e extensão real** (`.jpg`, `.png`, `.webp`, etc.) — não assumir `.png`.
...
- Escreva o conteúdo em **`profile/README.md`** (arquivo de exibição da organização).
